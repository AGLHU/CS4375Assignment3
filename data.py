...
class CrackerBox(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Attempting multiple possible paths for the image
        filename_img = os.path.join(self.data_path, os.path.basename(filename_gt.replace('-box.txt', '.jpg')))
        
        if not os.path.exists(filename_img):
            filename_img_alternate = os.path.join(self.data_path, os.path.basename(filename_gt.replace('.txt', '.jpg')))
            if os.path.exists(filename_img_alternate):
                filename_img = filename_img_alternate
            else:
                raise FileNotFoundError(f"Image file not found. Tried: {filename_img} and {filename_img_alternate}")

        # Load image and add a check
        image = cv2.imread(filename_img)
        if image is None:
            raise FileNotFoundError(f"Image file not found or unreadable: {filename_img}")
        ...
        # Ground truth processing (as per original code)
        ...
        # Scaling and setting bounding box information
        x1 *= self.scale_width
        ...
        if grid_x < self.yolo_grid_num and grid_y < self.yolo_grid_num:
            ...
            gt_box_blob[:, grid_y, grid_x] = torch.tensor([cx_offset, cy_offset, w, h, 1.0])
