...
def compute_loss(output, pred_box, gt_box, gt_mask, num_boxes, num_classes, grid_size, image_size):
    ...
    # loss components
    loss_x = torch.sum(weight_coord * box_mask * torch.pow(gt_box[:, 0:1, :, :] - output[:, 0:5*num_boxes:5], 2.0))
    loss_y = torch.sum(weight_coord * box_mask * torch.pow(gt_box[:, 1:2, :, :] - output[:, 1:5*num_boxes:5], 2.0))
    loss_w = torch.sum(weight_coord * box_mask * torch.pow(torch.sqrt(gt_box[:, 2:3, :, :]) - torch.sqrt(output[:, 2:5*num_boxes:5]), 2.0))
    loss_h = torch.sum(weight_coord * box_mask * torch.pow(torch.sqrt(gt_box[:, 3:4, :, :]) - torch.sqrt(output[:, 3:5*num_boxes:5]), 2.0))

    loss_obj = torch.sum(box_mask * torch.pow(box_confidence - output[:, 4:5*num_boxes:5], 2.0))
    loss_noobj = torch.sum(weight_noobj * (1 - box_mask) * torch.pow(output[:, 4:5*num_boxes:5], 2.0))

    # class predictions
    gt_classes = gt_box[:, 4:, :, :]  # one-hot encoded classes
    pred_classes = output[:, 5*num_boxes:, :, :]
    loss_cls = torch.sum(box_mask * torch.pow(gt_classes - pred_classes, 2.0))
